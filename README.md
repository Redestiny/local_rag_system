...
- LLM Engine：
  ...
  - 本地模式：Ollama（如 `llama3`）

### 逻辑数据流

1. 前端发起聊天/检索请求到 FastAPI
2. 后端根据 `engine` 路由到 API 引擎或本地引擎
3. RAG 链路从 ChromaDB 召回相关知识片段
4. 生成结果返回前端，前端记录会话状态与响应耗时
...
- `OLLAMA_BASE_URLS`：本地推理服务地址候选列表
- `OLLAMA_DEFAULT_MODEL`：本地默认模型名
...
| 变量名 | 必填 | 示例值 | 说明 |
| --- | --- | --- | --- |
| `DEEPSEEK_API_KEY` | 是（使用 `api` 引擎时） | `your_deepseek_api_key` | DeepSeek API 密钥 |
| `OLLAMA_BASE_URLS` | 否 | `http://localhost:11434,http://host.docker.internal:11434` | 本地推理地址列表 |
| `OLLAMA_DEFAULT_MODEL` | 否 | `llama3` | 本地默认模型 |
| `OLLAMA_BASE_URL` | 否 | `http://host.docker.internal:11434` | 容器内访问宿主机 Ollama 的地址 |
...
├─ backend/
│  ├─ app/
│  │  ├─ api/          # API 路由层（聊天、文档、向量检索）
│  │  ├─ core/         # 配置与数据库连接
│  │  ├─ models/       # 关系型数据库模型
│  │  ├─ schemas/      # Pydantic 请求/响应模型
│  │  ├─ services/     # RAG 链路、向量库、Embedding 等服务
│  │  └─ main.py       # FastAPI 入口
│  ├─ requirements.txt
│  └─ Dockerfile
├─ frontend/
